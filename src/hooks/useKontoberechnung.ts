...
export function useKontoberechnung() {
  return useQuery({
    ...
    queryFn: async (): Promise<Kontoberechnung> => {
      const [beitraegeRes, gastsessionsRes, ausgabenRes] = await Promise.all([
      ...
      const saldo = beitraege + gastsessions - ausgabenBootkonto - erstattungen

      return { beitraege, gastsessions, ausgabenBootkonto, erstattungen, saldo }
    },
  })
}
